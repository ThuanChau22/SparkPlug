...
import StationStatus from "./station-status.js";
...
const schema = mongoose.Schema({
  ...
  status: {
    type: String,
    enum: Object.values(StationStatus.Status),
    required: true,
  },
  site_id: {
    type: Number,
...
schema.loadClass(class {
  static async getStatuses({ filter, sort, limit } = {}) {
    try {
  ...
  static async getLatestStatuses({ filter, sort, limit, cursor } = {}) {
    try {
      const aggregate = [];
      ...
      // Query
      const stationStatuses = await StationStatusHistory.aggregate(aggregate);
...
const StationStatusHistory = mongoose.model("station_status_history", schema);

export default StationStatusHistory;
